...
const SUMMARY_TASK_URL = "https://exbot.ai/api/v1/products/langing/youtube_summary";
...
function createTaskElement(task) {
    ...
    if (task.status === 'success') {
        // <div class="py-summary-mark">
        //     <svg class="py-summary-mark-success-svg" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><g stroke-width="0"></g><g stroke-linecap="round" stroke-linejoin="round"></g><g><path fill="#000000" d="M17.47 250.9C88.82 328.1 158 397.6 224.5 485.5c72.3-143.8 146.3-288.1 268.4-444.37L460 26.06C356.9 135.4 276.8 238.9 207.2 361.9c-48.4-43.6-126.62-105.3-174.38-137z"></path></g></svg>
        // </div>
...
function processSummaryResult(result) {
    ...
    tasks.forEach(task => {
        if (task.task_id === id) {
            task.status = 'success';
            if (result.wpLink?.link) {
                task.link = result.wpLink.link;
            }

            if (result.wpLink === 'Error') {
                task.warning = 'Error';
            }
...
function checkSummaryStatus(taskId) {
    const checkInterval = 1000; // 1 sec
    var checkCount = 0;
        ...
        return new Promise((resolve, reject) => {
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    if (
                        !!data.task.wpLink?.link ||
                        checkCount > 5 ||
                        data.task.wpLink === 'Error'
                    ) {
            ...
            .catch(error => {
                showErrorMessage('API error: ' + error.toString());
                hideSpinner();
                reject();
            });
        });
...
function checkAllTasks() {
    const timeInterval = 5000; // 5 sec
    const tasks = getTasksStorage();
    for (let task of tasks) {
        if (task.status === 'processing') {
            checkSummaryStatus(task.task_id)
                .then(() => {
                    setTimeout(checkAllTasks, timeInterval);
                })
        }
    }
}
...
async function sendSummaryTask() {
    chrome.tabs.query({active: true}, tabs => {
        ...
        if (!url) {
            findCardBtn.classList.remove('active');
            showErrorMessage('Error: Cannot access active tab url, please check extension permissions or restart chrome');
        }
    });
